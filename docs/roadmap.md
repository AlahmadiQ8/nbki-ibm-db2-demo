...
- [x] **Bronze, complete and verified** — all six tables, **27,307,478 rows**,
      landed as Delta in `lh_bronze` in **10m31s** (~43k rows/s). Verified
      77/77 by `scripts/17_verify_bronze.py`: row counts, exact control totals,
      Db2→Delta types, merge-key uniqueness, null profiles and value domains.
      The definition is generated by `scripts/19_make_bronze_copyjob.py` and
      committed at `fabric/cj_bronze_db2.json`.
- [ ] **Bronze audit columns** — Copy job supports per-row provenance
      (extraction time, workspace/job/run id, incremental window bounds, custom
...
### `05_verify.sh` cannot see a rolled-back delta

A trap found while building bronze, and worth knowing before trusting a green
verification run.

`05_verify.sh` decides whether the delta has been applied by looking for the 250
inserted `TRANSACTION_ID`s. That is sound for a clean apply. It is **not** sound
after a rollback:

- `08_apply_delta.sh --rollback` deletes the 250 inserts, so the ID probe reports
  "not applied" again;
- rollback is partial by design and **cannot restore the 50 rows it updated in
  place**;
- `delta_manifest.json` records `updates_change_amount: false`, so no control
  total moves either.

**A half-spent delta therefore passes 23/23 with nothing visibly wrong** — and
bronze would then be landed from a contaminated baseline that nothing downstream
would reveal.

`./scripts/18_assert_pristine.sh` closes it. No before-images are needed: the 50
updates set `ERROR_FLAGS` to one of three values absent from the base data, and
updating a row advances its `ROW CHANGE TIMESTAMP`. The base load writes the whole
table in one burst — observed span for 13.3M rows is about **fourteen seconds** —
so a row touched days later is unmistakable. The script measures that window from
the rows the delta never touches, so nothing is hard-coded.

Run it before landing anything from Db2.
...
- Two Microsoft doc pages **conflict** on whether Copy job supports
  ...
  So Copy job **offers** incremental on a watermark column, and it picked up the
  `ROW CHANGE TIMESTAMP` column this repo exists to provide.

  > ### It offers it. It does not do it.
  >
  > **Tested, and the incremental leg fails.** Minimal reproduction, one table,
  > 109 rows:
  >
  > ```
  ...
  > ```
  >
  > Identical failure with `writeBehavior: Upsert` and with `Append`, so the
  > fault is in the incremental **read** from Db2, not the merge. `db2diag.log`
  > records nothing. The only error is *"Operation on target
  > CopyJobActivityLoop failed … Inner activity name: ConditionalCopy"*. The
  > portal-built definition behaves the same, so it is not a hand-authoring
  > artefact.
  >
  > **A CDC Copy job therefore works exactly once** — every later run takes the
  > incremental path and fails. Re-land with
  > `./scripts/16_fabric_pipeline.sh --reland`.
  >
  > `jobMode: Batch` is not an escape hatch either: it fails at once because the
  > runtime wants a `checkpointName` that CDC mode derives from
  > `changeDataSettings` and that no exported definition carries.
  >
  > This entry has now been wrong in **both** directions — first "full load
  > only" from the capability matrix, then "incremental works" from the exported
  > JSON. Neither reading was a test. The rule stands, and it is now expensive
  > enough to have earned its place: **run it in the tenant, or do not say it.**
  >
  > The delta (`08_apply_delta.sh`) remains **unspent** — deliberately, and
  > `./scripts/18_assert_pristine.sh` proves it (3/3).
